...
class OverlayRenderer:
    ...
    def draw_detection_box(
        ...
        if plate:
            ...
            overlay = frame.copy()
            cv2.rectangle(
                overlay, (x1, panel_y), (x2, panel_y + panel_height), color, -1
            )
        ...
        if track_id is not None:
            ...
            cv2.putText(frame, id_text, (x1, y2 + 20), self.FONT_MONO, 0.5, color, 1)
    ...
    def draw_scan_lines(self, frame: np.ndarray, intensity: float = 0.03) -> np.ndarray:
        ...
        cv2.line(
            frame, (0, self.scanline_offset), (w, self.scanline_offset), (50, 50, 50), 1
        )
    ...
    def draw_metrics_panel(self, frame: np.ndarray, stats: Dict) -> np.ndarray:
        ...
        metrics = [
            ...
            (
                "ACTIVE TRACKS",
                f"{stats.get('active_tracks', 0)}",
                self.COLORS["tracking"],
            ),
            (
                "TOTAL DETECTED",
        ...
        for label, value, color in metrics:
            ...
            cv2.putText(
                frame, value, (panel_x + 150, y_offset), self.FONT_TITLE, 0.6, color, 2
            )
            y_offset += 25
    ...
    def draw_camera_info(
        self, frame: np.ndarray, camera_id: str, timestamp: str = None
    ) -> np.ndarray:
        ...
        cv2.putText(
            frame, timestamp, (w - 200, 28), self.FONT_MONO, 0.6, self.COLORS["text"], 1
        )

        # Live indicator
        pulse = int(127 + 127 * np.sin(datetime.datetime.now().timestamp() * 2))
        cv2.circle(frame, (w - 230, 22), 6, (0, pulse, 0), -1)
        cv2.putText(frame, "LIVE", (w - 220, 28), self.FONT_BODY, 0.6, (0, pulse, 0), 1)

        return frame

    def draw_tracking_trail(
        self, frame: np.ndarray, centroid_history: List[Tuple[float, float]]
    ) -> np.ndarray:
    ...
    def draw_bottom_panel(
        self, frame: np.ndarray, active_tracks: List[Dict]
    ) -> np.ndarray:
        ...
        overlay = frame.copy()
        cv2.rectangle(
            overlay, (0, h - panel_height), (w, h), self.COLORS["background"], -1
        )
        ...
        for i, track in enumerate(active_tracks[:4]):
            ...
            # Track box
            color = (
                self.COLORS["verified"]
                if track.get("verified")
                else self.COLORS["tracking"]
            )

            # Plate
            plate = track.get("plate", "UNKNOWN")
            cv2.putText(
                frame, plate, (x_offset, y_offset), self.FONT_TITLE, 0.5, color, 2
            )
    ...
    def draw_alert(
        self, frame: np.ndarray, message: str, alert_type: str = "warning"
    ) -> np.ndarray:
        """Draw alert message"""
        h, w = frame.shape[:2]
        ...
        overlay = frame.copy()
        cv2.rectangle(
            overlay, (w // 4, h // 2 - 30), (3 * w // 4, h // 2 + 30), color, -1
        )
        frame = cv2.addWeighted(frame, 0.7, overlay, 0.3, 0)
...
def render_anpr_overlay(
    frame: np.ndarray, result: Dict, camera_id: str = "CAM_001"
) -> np.ndarray:
    ...
    if tracks and any(t.get("plate") for t in tracks):
        frame = renderer.draw_alert(
            frame, f"VEHICLE DETECTED: {tracks[0].get('plate', 'UNKNOWN')}", "success"
        )
