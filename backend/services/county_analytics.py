...
from ..county_data import KENYA_COUNTIES, County, get_county_by_name
...
class CountyAnalyticsService:
    ...
    def _generate_seed_data(self) -> Dict[str, Dict]:
        ...
        for county in KENYA_COUNTIES:
            ...
            seed[county.name] = {
                ...
                "risk_score": min(
                    95, max(15, 40 + pop_factor * 30 + random.randint(-15, 25))
                ),
    ...
    def get_county_stats(
        self, county_name: str, time_period: str = "month"
    ) -> CountyIncidentStats:
    ...
    def _get_monthly_data(self, period_multiplier: float) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "month": month,
                "accidents": int(random.randint(15, 45) * period_multiplier),
            }
            for month in months
        ]
    ...
    def get_region_summary(self) -> List[Dict]:
        ...
        for region in regions_data:
            ...
            regions_data[region]["risk_score"] = round(
                regions_data[region]["risk_score"] / count, 1
            )
    ...
    def get_infrastructure_analysis(self) -> Dict:
        ...
        return {
            ...
            "summary": {
                "avg_paved_pct": round(
                    sum(r["paved_roads_pct"] for r in road_quality) / len(road_quality),
                    1,
                ),
                "avg_maintained_pct": round(
                    sum(r["maintained_roads_pct"] for r in road_quality)
                    / len(road_quality),
                    1,
                ),
            ...
            yearly_data.append(
                {
                    "year": year,
                    "accidents": int(
                        random.randint(500, 1500) * (county.population / 1000000)
                    ),
                    "violations": int(
                        random.randint(2000, 6000) * (county.population / 1000000)
                    ),
                    "fatalities": int(
                        random.randint(50, 200) * (county.population / 1000000)
                    ),
                    "injuries": int(
                        random.randint(200, 800) * (county.population / 1000000)
                    ),
