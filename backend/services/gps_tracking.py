...
class GPSTrackingService:
    ...
    async def update_position(
        ...
        if len(self.position_history[user_id]) > self.max_history_per_user:
            self.position_history[user_id] = self.position_history[user_id][
    ...
    async def get_all_positions(
        self, user_type: Optional[str] = None, active_within_seconds: int = 300
    ) -> List[Dict[str, Any]]:
    ...
    async def get_position_history(
        self, user_id: str, since: Optional[datetime] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
    ...
    async def get_route_to_destination(
        self, user_id: str, dest_latitude: float, dest_longitude: float
        ...
        distance = await self.calculate_distance(
            position.latitude, position.longitude, dest_latitude, dest_longitude
        )
        ...
        bearing = math.degrees(
            math.atan2(
                math.sin(math.radians(dest_longitude - position.longitude))
                * math.cos(math.radians(dest_latitude)),
                math.cos(math.radians(position.latitude))
                * math.sin(math.radians(dest_latitude))
                - math.sin(math.radians(position.latitude))
                * math.cos(math.radians(dest_latitude))
    ...
    async def cleanup_old_positions(self, hours: int = 24):
        ...
        for user_id in list(self.position_history.keys()):
            self.position_history[user_id] = [
                p for p in self.position_history[user_id] if p.timestamp >= cutoff
            ]
