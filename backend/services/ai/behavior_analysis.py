...
class SpeedEstimator:
    ...
    def estimate_speed(
        ...
        meters_per_pixel = self.calibration.road_length_in_frame / self.calibration.pixels_per_meter
...
class LaneAnalyzer:
    ...
    def __init__(self, calibration: CameraCalibration):
        self.calibration = calibration
        self.lane_boundaries: List[Tuple[float, float]] = []  # (left_x, right_x) per lane
    ...
    def check_lane_violation(
        ...
        current_lane: int,
        previous_lane: int,
        crossing_solid_line: bool = False
    ) -> Tuple[bool, str]:
...
class AccidentDetector:
    ...
    def __init__(self):
        self.stopped_vehicles: Dict[str, Dict] = {}  # object_id -> {location, timestamp, has_people}
    ...
    def update(
        ...
        for obj in tracked_objects:
            if obj.is_stopped:
                ...
                # Check stop duration
                stop_duration = (current_time - 
                    self.stopped_vehicles[obj.object_id]["start_time"]).total_seconds()
        ...
        for i, obj_id1 in enumerate(stopped_ids):
            for obj_id2 in stopped_ids[i+1:]:
                if self._check_proximity(obj_id1, obj_id2):
                    ...
                    if self.stopped_vehicles[obj_id1].get("has_people_nearby") or \
                       self.stopped_vehicles[obj_id2].get("has_people_nearby"):
                        events.append(DetectionEvent(
                            event_type="accident",
                            camera_id="",
...
class HazardDetector:
    ...
    def __init__(self):
        self.static_objects: Dict[str, Dict] = {}  # object_id -> {position, timestamp, class}
        self.hazard_threshold_seconds = 10.0
        self.hazard_classes = {"debris", "pothole", "object", "fallen_tree", "broken_glass"}
    ...
    def update(
        ...
        for det in detections:
            ...
            if class_name in {"car", "truck", "bus", "motorcycle", "bicycle", "person", "pedestrian"}:
                continue
            ...
            if class_name.lower() in self.hazard_classes:
                if obj_id not in self.static_objects:
                    ...
                    duration = (current_time - 
                        self.static_objects[obj_id]["start_time"]).total_seconds()
    ...
    def process_frame(
        ...
        for det in detections:
            ...
            if class_name in {"car", "truck", "bus", "motorcycle"}:
                ...
                # Check overspeeding
                is_overspeeding, severity = self.speed_estimator.check_overspeeding(speed)
                if is_overspeeding:
                    events.append(DetectionEvent(
                        ...
                        location={"lat": self.calibration.lat, "lng": self.calibration.lng},
                        description=f"Vehicle traveling at {speed:.1f} km/h in {self.calibration.speed_limit} km/h zone",
                        evidence={"speed": speed, "limit": self.calibration.speed_limit}
                    ))
                ...
                if speed < 1.0:
                    ...
                    if obj.previous_positions:
                        ...
                        obj.stopped_duration = (timestamp - last_pos_time).total_seconds()
                else:
                ...
                if obj_id in self.previous_lanes:
                    ...
                    if is_violation:
                        events.append(DetectionEvent(
                            ...
                            confidence=confidence,
                            location={"lat": self.calibration.lat, "lng": self.calibration.lng},
                            description=f"Vehicle changed lanes illegally",
                            evidence={"from_lane": prev_lane, "to_lane": current_lane}
                        ))
        ...
        accident_events = self.accident_detector.update(tracked_list, pedestrian_positions)
