        ...
        for det in detections:
            ...
            if best_match_id is not None:
                ...
                if len(self.tracks[best_match_id]["position_history"]) > 30:
                    self.tracks[best_match_id]["position_history"] = self.tracks[
                        best_match_id
                    ]["position_history"][-30:]
            else:
                # New track
...
class AIDetectionPipeline:
    ...
    async def process_frame(
        self, camera_id: str, frame: np.ndarray, frame_number: int
    ) -> DetectionResult:
            ...
            try:
                ...
                for result in results:
                    ...
                    for box in boxes:
                        ...
                        if (
                            class_name not in self.vehicle_classes
                            and class_name not in self.person_classes
                        ):
                            continue
    ...
    def detect_accident(
        ...
        pedestrians = sum(1 for d in all_detections if d.class_name == "pedestrian")
