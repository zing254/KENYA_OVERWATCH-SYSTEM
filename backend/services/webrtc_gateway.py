...
class WebRTCGateway:
    ...
    async def create_stream(
        ...
        if len(self.active_streams) >= self.max_concurrent_streams:
            return {"success": False, "error": "Maximum concurrent streams reached"}
    ...
    async def handle_frame(
        self, stream_id: str, frame_data: bytes, frame_info: Dict
    ) -> bool:
                ...
                await self.stream_callbacks[stream_id](
                    {
                        ...
                        "frame": frame_data,
                        "info": frame_info,
                        "location": {"lat": stream.latitude, "lng": stream.longitude},
                    }
                )
    ...
    def register_frame_callback(self, stream_id: str, callback: Callable):
...
class SignalingServer:
    ...
    async def handle_offer(self, stream_id: str, offer: Dict, client_id: str) -> Dict:
        ...
        return {"type": "answer", "sdp": "placeholder_sdp_answer"}

    async def handle_ice_candidate(
        self, stream_id: str, candidate: Dict, client_id: str
    ):
    ...
    async def notify_viewers(self, stream_id: str, event: str, data: Dict):
