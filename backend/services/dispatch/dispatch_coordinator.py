...
class DispatchCoordinator:
    ...
    def dispatch_responders(
        ...
        logger.info(f"Dispatched {len(dispatches)} responders to incident {incident_id}")
    ...
    def mark_enroute(self, dispatch_id: str) -> bool:
        ...
        logger.info(f"Responder {dispatch.responder_id} en route to incident {dispatch.incident_id}")
        return True
    ...
    def mark_onscene(self, dispatch_id: str) -> bool:
        ...
        logger.info(f"Responder {dispatch.responder_id} on scene at incident {dispatch.incident_id}")
        return True
    ...
    def resolve_dispatch(self, dispatch_id: str) -> bool:
        ...
        incident_dispatches = [
            ...
            if d.incident_id == dispatch.incident_id
        ]
        
        all_resolved = all(
            d.status == DispatchStatus.RESOLVED 
            for d in incident_dispatches
        )
    ...
    def reject_dispatch(self, dispatch_id: str, reason: str = "") -> bool:
        ...
        if incident:
            ...
            if alternative:
                ...
                logger.info(f"Reassigned {dispatch.responder_type.value} to incident {dispatch.incident_id}")
