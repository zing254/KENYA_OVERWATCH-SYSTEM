...
class NotificationService:
    ...
    def format_message(
        self, template_name: str, notification_type: NotificationType, **kwargs
    ) -> Dict[str, str]:
        ...
        if notification_type in [NotificationType.EMAIL, NotificationType.ALL]:
            ...
            result["subject"] = template.get("subject", "NTSA Notification").format(
                **kwargs
            )
    ...
    async def send_violation_notification(
        self, violation: Dict, vehicle: Optional[Dict] = None
    ):
        ...
        owner_name = (
            vehicle.get("owner_name", "Vehicle Owner") if vehicle else "Vehicle Owner"
        )
    ...
    async def send_accident_notification(self, accident: Dict):
        ...
        return await self.send_bulk(
            recipients=["+254700000001", "+254700000002"], **messages
        )
    ...
    async def send(
        ...
        results = {"sent_at": datetime.now().isoformat(), "channels": []}
        ...
        if self.sms_enabled and phone and sms:
            try:
                ...
                results["channels"].append(
                    {"type": "sms", "status": "failed", "error": str(e)}
                )
        ...
        if self.email_enabled and email and email_body:
            try:
                ...
                result = await email_service.send_email(
                    to=email, subject=subject or "NTSA Notification", body=email_body
                )
                results["channels"].append(
                    {
                        "type": "email",
        ...
        # Log to history
        self.history.append({**results, "phone": phone, "email": email})
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "by_type": {
                ...
                "email": len(
                    [h for h in self.history if "email" in h.get("channels", [])]
                ),
            },
        }
