    ...
    def __init__(self, sounds_dir: str = "/data/sounds"):
        ...
        self.alerts_config: Dict[AlertSound, AlertConfig] = {
            ...
            AlertSound.CAMERA_OFFLINE: AlertConfig(
                name="Camera Offline",
                sound_file="offline.mp3",
                volume=0.5,
                repeat=2
            ),
            AlertSound.EMERGENCY: AlertConfig(
                name="Emergency",
                sound_file="emergency.mp3",
                volume=1.0,
                repeat=5
            ),
        }
                ...
                try:
                    ...
                    if alert_type in self.alerts_config:
                        if "volume" in alert_config:
                            self.alerts_config[alert_type].volume = alert_config["volume"]
                        if "enabled" in alert_config:
                            self.alerts_config[alert_type].enabled = alert_config["enabled"]
                        if "repeat" in alert_config:
                            self.alerts_config[alert_type].repeat = alert_config["repeat"]
                except ValueError:
                    pass
...
class RadioCallSimulator:
    ...
    def simulate_dispatch(self, team_name: str, location: str, priority: str, eta: int) -> list[str]:
