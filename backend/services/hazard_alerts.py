    ...
    def _generate_initial_alerts(self):
        ...
        alert_templates = [
            (
                ...
                AlertPriority.HIGH,
                "Flash Flood Warning",
                "Heavy rainfall has been detected. Possible flooding on nearby roads.",
            ),
            (
                AlertCategory.LANDSLIDE,
                AlertPriority.CRITICAL,
                "Landslide Risk",
                "Soil saturation levels high. Landslide possible on steep terrain.",
            ),
            (
                AlertCategory.ROAD_DAMAGE,
                AlertPriority.MEDIUM,
                "Road Surface Damage",
                "Pothole or road damage reported. Exercise caution.",
            ),
            (
                AlertCategory.WEATHER,
                AlertPriority.HIGH,
                "Severe Weather Alert",
                "Thunderstorms expected. Reduced visibility on roads.",
            ),
            (
                AlertCategory.TRAFFIC,
                AlertPriority.MEDIUM,
                "Heavy Traffic",
                "Congestion detected. Expect delays.",
            ),
            (
                AlertCategory.INFRASTRUCTURE,
                AlertPriority.LOW,
                "Road Work",
                "Construction work ongoing. Follow diversion signs.",
            ),
        ]
        ...
        return sorted(
            ...
            key=lambda x: (
                ...
                if x.priority == AlertPriority.CRITICAL
                    ...
                    1
                    if x.priority == AlertPriority.HIGH
                    else 2 if x.priority == AlertPriority.MEDIUM else 3
                )
            ),
        )
    ...
    def get_alert_summary(self) -> Dict:
        return {
            ...
            "by_priority": {
                ...
                "high": len(
                    [a for a in self.active_alerts if a.priority == AlertPriority.HIGH]
                ),
                "medium": len(
                    [
                        a
                        for a in self.active_alerts
                        if a.priority == AlertPriority.MEDIUM
                    ]
                ),
                "low": len(
                    [a for a in self.active_alerts if a.priority == AlertPriority.LOW]
                ),
            },
            "by_category": {
