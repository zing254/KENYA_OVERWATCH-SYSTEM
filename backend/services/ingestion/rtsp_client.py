...
class RTSPClient:
    ...
    def _capture_loop(self):
        ...
        while self.running:
            try:
                ...
                if self.cap is None or not self.cap.isOpened():
                    logger.warning(f"Camera {self.config.id}: Capture device not open, reconnecting...")
                    if not self.connect():
                        break
                    continue
    ...
    def stop(self):
        ...
        if self.cap:
            self.cap.release()
        logger.info(f"Camera {self.config.id}: Stopped (dropped {self.dropped_frames} frames)")
    ...
    def get_service_stats(self) -> Dict:
        ...
        with self._lock:
            return {
                ...
                "overall_drop_rate": (self.stats["total_dropped"] / max(self.stats["total_frames"], 1)) * 100,
            }
