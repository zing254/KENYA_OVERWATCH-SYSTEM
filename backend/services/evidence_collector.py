...
class EvidenceCollector:
    ...
    async def generate_report(
        ...
        report = {
            ...
            "summary": {
                "total_screenshots": len([e for e in evidence_list if e.get("type") == "screenshot"]),
                "total_videos": len([e for e in evidence_list if e.get("type") == "video"]),
    ...
    async def cleanup_old_evidence(self, days: int = 90):
        ...
        for filepath in self.storage_path.glob("*"):
            if filepath.is_file():
                ...
                if mtime < cutoff:
                    filepath.unlink()
                    print(f"Deleted old evidence: {filepath.name}")
