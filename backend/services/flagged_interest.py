...
class FlaggedInterestService:
    ...
    async def check_reidentification(
        ...
        for flagged_id, flagged in self.flagged_interests.items():
            if (
                flagged.plate_number == plate_upper
                and flagged.status == FlaggedStatus.ACTIVE
            ):
            ...
            results.append(
                {
                    ...
                    "last_seen": {
                        ...
                        "timestamp": (
                            flagged.last_seen_timestamp.isoformat()
                            if flagged.last_seen_timestamp
                            else None
                        ),
                    },
    ...
    async def get_reidentification_history(
        self, flagged_id: Optional[str] = None, limit: int = 50
    ) -> List[Dict]:
    ...
    async def get_nearest_available_team(
        self, latitude: float, longitude: float, teams: List[Dict]
    ) -> Optional[Dict]:
        ...
        nearest = min(
            ...
            key=lambda t: distance(
                latitude, longitude, t.get("latitude", 0), t.get("longitude", 0)
            ),
        )
