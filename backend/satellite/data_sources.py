...
class SatelliteDataSource:
    ...
    def _generate_sample_data(self):
        for i in range(100):
            ...
            sensor = SatelliteSensor.MSI if satellite_type in [SatelliteType.SENTINEL_2, SatelliteType.LANDSAT_8, SatelliteType.LANDSAT_9] else SatelliteSensor.SAR
            
            self.images.append(SatelliteImage(
                satellite_id=f"SAT-{i+1:04d}",
                satellite_type=satellite_type,
                sensor=sensor,
                acquisition_date=datetime.now(timezone.utc) - timedelta(days=random.randint(0, 30)),
                latitude=lat,
                longitude=lon,
                cloud_coverage=random.uniform(0, 80) if sensor != SatelliteSensor.SAR else 0,
                spatial_resolution=10 if satellite_type == SatelliteType.SENTINEL_2 else 30,
        ...
        for i in range(50):
            ...
            self.hazards.append(HazardDetection(
                id=f"HAZ-{i+1:04d}",
                hazard_type=hazard_type,
                satellite_id=f"SAT-{random.randint(1, 100):04d}",
                detection_date=datetime.now(timezone.utc) - timedelta(days=random.randint(0, 14)),
                latitude=lat,
                longitude=lon,
                severity=severity,
                confidence=random.uniform(0.65, 0.98),
                affected_area_km2=random.uniform(0.5, 25.0),
                description=self._generate_hazard_description(hazard_type, severity),
                nearby_roads=self._get_nearby_roads(lat, lon),
                nearby_county=random.choice(counties),
                status="active"
            ))
    
    def _generate_hazard_description(self, hazard_type: HazardType, severity: str) -> str:
        descriptions = {
            ...
            HazardType.WATER_ACCUMULATION: f"Standing water detected on road surface. May indicate drainage issues.",
            HazardType.EROSION: f"Road shoulder erosion detected. Vegetation loss observed in adjacent areas.",
            HazardType.VEGETATION_ENCROACHMENT: f"Vegetation growth encroaching on roadway. May affect visibility and road width."
        }
    ...
    def _get_nearby_roads(self, lat: float, lon: float) -> List[str]:
        major_roads = [
            "Nairobi-Mombasa Highway", "Nairobi-Nakuru Highway", "Nairobi-Thika Highway",
            "Mombasa-Malindi Road", "Nairobi-Narok Road", "Nairobi-Addis Ababa Road",
            "Eldoret-Webuye Road", "Kisumu-Busia Road", "Nyeri-Marsabit Road",
            "Nakuru-Eldoret Highway", "Nairobi-Garissa Road", "Mombasa-Tanga Road"
        ]
