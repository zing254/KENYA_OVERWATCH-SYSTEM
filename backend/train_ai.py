        ...
        if full_path.exists():
            if subdirs:
                for subdir in subdirs:
                    ...
                    if sub_path.exists():
                        images = list(sub_path.glob("*.jpg")) + list(
                            sub_path.glob("*.png")
                        )
...
""")
    ...
    try:
        ...
        print("✓ Training complete! Model saved to runs/train/kenya_overwatch")
...
def export_model(
    model_path="runs/train/kenya_overwatch/weights/best.pt", format="onnx"
):
