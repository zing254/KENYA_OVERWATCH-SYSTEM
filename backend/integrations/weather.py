...
class WeatherService:
    ...
    def _generate_weather_data(self):
        ...
        for county in counties:
            ...
            self.weather_data[county] = WeatherData(
                ...
                precipitation_mm=(
                    random.uniform(0, 15) if WeatherCondition.RAIN in conditions else 0
                ),
    ...
    def _generate_alerts(self):
        alert_types = [
            (
                WeatherAlert.FLOOD_WARNING,
                "Flood",
                ["Kisumu", "Homa Bay", "Siaya", "Mombasa"],
            ),
            (
                WeatherAlert.HEAVY_RAIN,
                "Rain",
                ["Nairobi", "Kakamega", "Kericho", "Nakuru"],
            ),
            (
                WeatherAlert.LANDSLIDE_RISK,
                "Landslide",
                ["Kericho", "Kakamega", "Meru", "Nyeri"],
            ),
            (
                WeatherAlert.ROAD_SLIPPERY,
                "Slippery",
                ["Nairobi", "Nakuru", "Eldoret", "Kericho"],
            ),
        ]
            ...
            for county in counties[:2]:
                self.alerts.append(
                    WeatherAlertData(
                        ...
                        + timedelta(hours=random.randint(6, 24)),
    ...
    def get_forecast(self, county: str, days: int = 5) -> Dict:
            ...
            forecast.append(
                {
                    ...
                    "temperature_high_c": round(
                        base.temperature_c + temp_variation + random.uniform(2, 5), 1
                    ),
                    "temperature_low_c": round(
                        base.temperature_c + temp_variation - random.uniform(2, 5), 1
                    ),
        ...
        return {"county": county, "forecast_days": days, "forecast": forecast}
