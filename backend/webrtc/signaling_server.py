...
class WebRTCSignalingServer:
            ...
            async for message in websocket:
                try:
                    ...
                    if msg_type == "register_phone":
                        ...
                        client_id = await self._handle_dashboard_register(websocket, data)
                    elif msg_type == "offer":
    ...
    async def _handle_phone_register(self, websocket: websockets.WebSocketServerProtocol, data: dict) -> str:
    ...
    async def _handle_dashboard_register(self, websocket: websockets.WebSocketServerProtocol, data: dict) -> str:
    ...
    async def _handle_answer(self, client_id: str, data: dict):
        ...
        if target and target in self.connected_phones:
            await self.connected_phones[target].send(json.dumps({
                "type": "answer",
                "sdp": sdp_answer
            }))
    ...
    async def _handle_ice_candidate(self, client_id: str, data: dict):
        ...
        if target:
            if client_id in self.connected_phones and target in self.connected_dashboards:
                await self.connected_dashboards[target].send(json.dumps({
                    "type": "candidate",
                    "phone_id": client_id,
                    "candidate": candidate
                }))
            elif client_id in self.connected_dashboards and target in self.connected_phones:
                await self.connected_phones[target].send(json.dumps({
                    "type": "candidate",
                    "candidate": candidate
                }))
    ...
    async def _handle_subscribe(self, client_id: str, data: dict):
        ...
        if client_id in self.dashboard_subscriptions:
            ...
            await self._send_to_dashboard(client_id, {
