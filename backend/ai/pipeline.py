...
# DetectionType may not exist; provide a fallback
try:
...
class ObjectTracker:
    ...
    def update(
        self, bounding_boxes: List[Tuple[BoundingBox, DetectionType, float]]
    ) -> Dict[int, Tuple[BoundingBox, DetectionType, float, int]]:
        ...
        D = np.linalg.norm(
            np.asarray(track_centroids)[:, np.newaxis] - np.asarray(input_centroids),
            axis=2,
        )
        ...
        for row, col in zip(rows, cols):
            ...
            track_mapping[track_id] = (
                bbox,
                self.tracks[track_id]["detection_type"],
                conf,
                track_id,
            )
...
class MotionDetector:
    ...
    def detect_motion(
        ...
        motion_ratio = (
            sum(b.area for b in motion_boxes) / (frame.shape[0] * frame.shape[1])
            if motion_boxes
            else 0
        )
...
class AnomalyDetector:
    ...
    def update_baseline(
        self, camera_id: str, detection_count: int, timestamp: datetime
    ):
        ...
        self.time_windows[camera_id].append(
            {"count": detection_count, "timestamp": timestamp}
        )
    ...
    def detect_anomaly(self, camera_id: str, detection_count: int) -> Tuple[bool, str]:
        ...
        if detection_count > stats["threshold_high"]:
            return (
                True,
                f"Unusually high activity: {detection_count} vs baseline {stats['mean']:.1f}",
            )

        if detection_count < stats["threshold_low"]:
            return (
                True,
                f"Unusually low activity: {detection_count} vs baseline {stats['mean']:.1f}",
            )

        return False, ""

    def detect_loitering(
        self, detections: List[Detection], time_window_seconds: int = 60
    ) -> bool:
        ...
        recent = [
            d
            for d in detections
            if (now - d.timestamp).total_seconds() < time_window_seconds
        ]
        ...
        position_variance = np.var([p[0] for p in positions]) + np.var(
    ...
    def detect_abandoned_object(
        self, detections: List[Detection], min_duration_seconds: int = 30
    ) -> Optional[Detection]:
...
class HeatmapGenerator:
    ...
    def add_detection(self, camera_id: str, detection: Detection):
        ...
        if camera_id not in self.heatmaps:
            self.heatmaps[camera_id] = np.zeros(
...
class AIPipeline:
    ...
    def __init__(self):
        ...
        self.motion_detector = MotionDetector() if cv2 is not None else None
        ...
        self._available = cv2 is not None and torch is not None
    ...
    def _track_detections(
        self, detections: List[Detection], camera_id: str
    ) -> Dict[int, Detection]:
        """Track detections"""
...
class TrafficAnalyzer:
    ...
    def analyze_vehicle(
        ...
        positions.append({"x": center[0], "y": center[1], "timestamp": frame_timestamp})
            ...
            if dt > 0:
                ...
                if len(speeds) >= 5:
                    ...
                    if avg_speed < 2 and track_key not in self.stopped_vehicles:
                        ...
                        stopped_duration = (
                            frame_timestamp - self.stopped_vehicles[track_key]
                        ).total_seconds()
        ...
        for d in detections:
            if d.detection_type == DetectionType.VEHICLE:
                ...
                if track_key in self.stopped_vehicles:
                    stopped_duration = (
                        now - self.stopped_vehicles[track_key]
                    ).total_seconds()
    ...
    def detect_hazard(
        self, detections: List[Detection], stationary_seconds: int = 10
    ) -> List[Detection]:
        """Detect road hazards"""
        hazards = []
        now = datetime.now()
