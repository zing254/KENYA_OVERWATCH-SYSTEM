...
def _accident_to_dict(accident) -> Dict:
    ...
    return {
        "id": accident.id,
        "accident_type": (
            accident.accident_type.value
            if hasattr(accident.accident_type, "value")
            else str(accident.accident_type)
        ),
        "cause": (
            accident.cause.value
            if hasattr(accident.cause, "value")
            else str(accident.cause)
        ),
        ...
        "severity": (
            ...
            if hasattr(accident.severity, "value")
            else str(accident.severity)
        ),
        "status": (
            accident.status.value
            if hasattr(accident.status, "value")
            else str(accident.status)
        ),
        ...
        "reported_at": (
            accident.reported_at.isoformat() if accident.reported_at else None
        ),
...
def _violation_to_dict(violation) -> Dict:
    ...
    return {
        ...
        "vehicle_type": (
            violation.vehicle_type.value
            if hasattr(violation.vehicle_type, "value")
            else str(violation.vehicle_type)
        ),
        ...
        "status": (
            ...
            if hasattr(violation.status, "value")
            else str(violation.status)
        ),
        ...
        "detected_at": (
            violation.detected_at.isoformat() if violation.detected_at else None
        ),
...
def get_accidents(filters: Optional[Dict] = None, limit: int = 100) -> List[Dict]:
    ...
    if filters:
        ...
        if "severity" in filters:
            accidents = [
                a for a in accidents if a.get("severity") == filters["severity"]
            ]
        if "plate_number" in filters:
            accidents = [
                a
                for a in accidents
                if filters["plate_number"] in a.get("vehicles_involved", [])
            ]
    ...
    if filters:
        ...
        if "plate_number" in filters:
            violations = [
                v
                for v in violations
                if v.get("plate_number") == filters["plate_number"]
            ]
        if "violation_type" in filters:
            violations = [
                v
                for v in violations
                if v.get("violation_type") == filters["violation_type"]
            ]
