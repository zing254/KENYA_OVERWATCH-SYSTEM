    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <header className="bg-gradient-to-r from-ntsa-primaryDark via-ntsa-primary to-ntsa-primaryDark border-b-2 border-ntsa-primaryLight/30 sticky top-0 z-20">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <button onClick={() => setShowSidebar(!showSidebar)} className="p-1 hover:bg-ntsa-primaryLight/20 rounded">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="relative p-2 hover:bg-ntsa-primaryLight/20 rounded"
            >
        ...
        {/* Tab Navigation - NTSA Theme */}
        <div className="flex border-b border-ntsa-primaryLight/20 overflow-x-auto">
          {tabs.map(tab => (
            <button
              key={tab.id}
              ...
              className={`flex-1 py-3 px-2 text-xs font-medium flex items-center justify-center gap-1 whitespace-nowrap ${
                activeTab === tab.id 
                  ? 'text-ntsa-primaryLight border-b-2 border-ntsa-primaryLight bg-ntsa-primaryDark/50' 
                  : 'text-gray-400 hover:bg-ntsa-primaryDark/30'
              }`}
            >
      ...
      {showSidebar && (
        <div className="fixed inset-0 z-30 flex">
          ...
          <div className="fixed left-0 top-0 bottom-0 w-64 bg-gradient-to-b from-ntsa-primaryDark to-gray-900 border-r border-ntsa-primaryLight/20 p-4 transform transition-transform">
            ...
            <div className="space-y-2">
              <button onClick={() => { setActiveTab('dashboard'); setShowSidebar(false) }} className="w-full flex items-center gap-3 p-3 hover:bg-ntsa-primary/50 rounded-lg text-white">
                <Shield className="w-5 h-5 text-ntsa-primaryLight" /> Dashboard
              </button>
              <button onClick={() => { setActiveTab('incidents'); setShowSidebar(false) }} className="w-full flex items-center gap-3 p-3 hover:bg-ntsa-primary/50 rounded-lg text-white">
                <AlertTriangle className="w-5 h-5 text-red-400" /> Incidents
              </button>
              <button onClick={() => { setActiveTab('map'); setShowSidebar(false) }} className="w-full flex items-center gap-3 p-3 hover:bg-ntsa-primary/50 rounded-lg text-white">
                <MapPin className="w-5 h-5 text-green-400" /> Map
              </button>
              <button onClick={() => { setActiveTab('team'); setShowSidebar(false) }} className="w-full flex items-center gap-3 p-3 hover:bg-ntsa-primary/50 rounded-lg text-white">
                <Users className="w-5 h-5 text-purple-400" /> My Team
              </button>
              <button onClick={() => { setActiveTab('chat'); setShowSidebar(false) }} className="w-full flex items-center gap-3 p-3 hover:bg-ntsa-primary/50 rounded-lg text-white">
                <MessageSquare className="w-5 h-5 text-yellow-400" /> Chat
              </button>
              <hr className="border-ntsa-primaryLight/20 my-4" />
              <button className="w-full flex items-center gap-3 p-3 hover:bg-ntsa-primary/50 rounded-lg text-gray-300">
                <Settings className="w-5 h-5" /> Settings
              </button>
              <button className="w-full flex items-center gap-3 p-3 hover:bg-red-500/20 rounded-lg text-red-400">
